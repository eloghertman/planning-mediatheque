...
# Exception validée (demande utilisatrice 11/09) : un·e agent·e peut être
# noté·e en congé/absence tout en assistant à LA réunion pôle hebdomadaire
# (Anne-Françoise, Stéphanie, Delphine, Marie-France, Eloïse) — ce n'est
# jamais une anomalie, quel que soit l'agent concerné. Aucune autre réunion
# n'est concernée par cette exception.
EXCEPTION_REUNION_ABSENCE = 'reunion pole'
...
def parse_note_horaire(texte):
    """Si une note agent (zone libre W-Z) commence par un horaire explicite
    tapé par l'agent lui-même (ex. '17h30-19h absence' ou '13h30-15h réunion
    acquisition'), retourne (debut, fin, reste_du_texte) en minutes.
    Sinon None.

    Cette note est la SOURCE LA PLUS PRÉCISE dont on dispose pour l'horaire
    réel d'un événement : la case Accueil/Réunion/Absence affichée dans la
    grille (colonnes H/I/J) ne fait souvent que répéter le même texte sur
    TOUTES les lignes d'une journée (fusion de cellules ou recopie), sans
    préciser l'horaire réel si l'agent ne l'a pas indiqué entre parenthèses
    dans cette case — alors que sa note d'origine, elle, le précise
    toujours en tête (format demandé : 'HHh(MM)-HHh(MM) texte')."""
    m = re.match(r'^(\d{1,2})h(\d{2})?-(\d{1,2})h(\d{2})?\s+(.+)$', (texte or '').strip())
    if not m:
        return None
    h1, m1, h2, m2, reste = m.groups()
    debut = int(h1) * 60 + int(m1 or 0)
    fin = int(h2) * 60 + int(m2 or 0)
    return debut, fin, reste.strip()
...
def construire_occurrences_jour(jour_data, agents_connus, notes=None):
    # Index des notes agents (zone libre W-Z) qui commencent par un horaire
    # explicite, par agent normalisé — voir parse_note_horaire(). Sert à
    # corriger l'horaire d'une occurrence Accueil/Réunion/Absence quand la
    # case correspondante affiche le même texte sur plusieurs lignes de la
    # journée (cellule fusionnée, ou recopie automatique) : dans ce cas,
    # l'horaire RÉEL n'est connu que par la note d'origine de l'agent, pas
    # par la ligne où le texte apparaît dans la grille — c'est donc la note
    # qui fait foi dès qu'elle correspond au texte de la case (découvert le
    # 11/09 : plusieurs fausses alertes venaient de cases Absence/Réunion
    # fusionnées sur toute la journée alors que la note de l'agent précisait
    # un horaire bien plus court).
    notes_par_agent = defaultdict(list)
    for agent, texte in (notes or []):
        parsed = parse_note_horaire(texte)
        if parsed:
            notes_par_agent[normalize(agent)].append(parsed)

    def preciser(agent, d, f, seg):
        for (nd, nf, reste) in notes_par_agent.get(normalize(agent), []):
            if reste and normalize(reste) in normalize(seg):
                return nd, nf
        return d, f
...
# ─────────────────────────────────────────────────────────────
#  PLANNING TYPE — recherche partagée (règles R1/R4 samedi et R10)
# ─────────────────────────────────────────────────────────────

def construire_pt_blocs(prep, jour, jour_cap, jour_data):
    """Retourne la liste des blocs horaires du planning type applicables à
    ce jour — [(debut_min, fin_min, {section: [agents]}), ...] — ou [] si
    aucun planning type n'est disponible ou ne couvre ce jour. Gère le
    samedi bleu/rouge (clé 'Samedi_BLEU'/'Samedi_ROUGE'). Factorisé ici
    (11/09) pour être utilisé à la fois par la règle R10 (couverture) et
    par la règle samedi ci-dessous (le planning type l'emporte sur
    Horaires_Des_Agents en cas de conflit)."""
...
SECTION_PT_KEY = {'RDC': 'RDC', 'Adulte': 'Adulte', 'M & F': 'MF', 'Jeunesse': 'Jeunesse'}


def pt_couvre_occurrence(agent, occ, pt_blocs):
    """Vrai si le planning type place CET agent (même nom, pas juste la
    section couverte par quelqu'un — demande utilisatrice 11/09 : un
    remplacement par un·e autre agent·e n'est PAS couvert par cette
    règle, seule une correspondance exacte avec le planning type l'est),
    de façon continue, sur toute la durée de l'occurrence. Gère le cas où
    l'occurrence réelle (ex. texte fusionné sur plusieurs heures) chevauche
    plusieurs blocs horaires du planning type (souvent découpé heure par
    heure). Ne concerne que RDC/Adulte/M&F/Jeunesse — le planning type ne
    dit rien des Accueil/Réunion/Absence, donc toujours False pour ces
    types-là (retombe sur la vérification normale)."""
    cle = SECTION_PT_KEY.get(occ['type'])
    if cle is None or not pt_blocs:
        return False
    cs, ce = occ['debut'], occ['fin']
    chevauchants = sorted(
        (max(bcs, cs), min(bce, ce), sections_agents)
        for bcs, bce, sections_agents in pt_blocs
        if bcs < ce and bce > cs
    )
    if not chevauchants:
        return False
    curseur = cs
    for bs, be, sections_agents in chevauchants:
        agents_bloc = [a for a in sections_agents.get(cle, []) if a and a.strip()]
        if bs > curseur or agent not in agents_bloc:
            return False
        curseur = max(curseur, be)
    return curseur >= ce


def verifier_pause_samedi(agent, occs_travail, pause_flex, semaine_label, jour, anomalies):
    """Règle samedi (demande utilisatrice 11/09) : indépendamment de la
    répartition déclarée dans Horaires_Des_Agents pour ce jour (qui n'est
    pas fiable le samedi), chaque agent doit avoir au moins 1h consécutive
    sans occupation entre 12h et 14h. Exception : les agent·es à pause
    flexible peuvent décaler cette coupure jusqu'à 14h (fenêtre élargie
    12h-15h) — iels peuvent donc travailler sans interruption de 12h à
    14h du moment que la coupure d'1h suit immédiatement."""
    fenetre = (PAUSE_FENETRE[0], 15 * 60 if agent in pause_flex else PAUSE_FENETRE[1])
    segs = sorted(
        (max(o['debut'], fenetre[0]), min(o['fin'], fenetre[1]))
        for o in occs_travail if o['debut'] < fenetre[1] and o['fin'] > fenetre[0]
    )
    libre_max = 0
    curseur = fenetre[0]
    for d, f in segs:
    ...
    libre_max = max(libre_max, fenetre[1] - curseur)
    if libre_max < PAUSE_MIN_LIBRE:
        anomalies.append(Anomalie(
            'rouge', semaine_label, jour,
            f"{agent} ne semble pas avoir 1h de pause déjeuner libre entre {fmt_min(fenetre[0])} et "
            f"{fmt_min(fenetre[1])} ce samedi (règle samedi : 1h de coupure entre 12h et 14h, décalable "
            f"jusqu'à 14h pour les pauses flexibles — indépendante des horaires habituels de la semaine).",
            'Pause déjeuner samedi'))
...
def verifier_jour(jour_data, semaine_label, semaine_num, vue_agent, agents_connus, prep, anomalies, notes=None):
    jour = jour_data['jour']
    jour_cap = JOUR_CAPITALISE.get(jour, jour.capitalize())
    date_str = jour_data.get('date_str')
    occ_brutes = construire_occurrences_jour(jour_data, agents_connus, notes)
    ...
    pt_blocs = construire_pt_blocs(prep, jour, jour_cap, jour_data) if prep else []
    ...
    for agent, liste in occ_brutes.items():
        ...
        if mode_complet and not est_vacataire(agent):
            ...
            if h is None:
                ...
                # Règle samedi (demande utilisatrice 11/09) : le planning
                # type l'emporte toujours en cas de conflit avec
                # Horaires_Des_Agents — si CETTE occurrence correspond
                # exactement à ce que prévoit le planning type pour cet
                # agent (même nom), elle est présumée correcte (arrivée,
                # départ) même si elle contredit Horaires_Des_Agents.
                # Sinon, seules l'arrivée et le départ restent vérifiés
                # ici via Horaires_Des_Agents (peu de raison qu'ils soient
                # en conflit avec le planning type en pratique) ; la pause
                # déjeuner, elle, suit une règle propre au samedi (1h
                # entre 12h et 14h, décalable à 14h en cas de pause
                # flexible) et n'est plus jugée sur la répartition
                # matin/après-midi déclarée dans Horaires_Des_Agents pour
                # ce jour — voir verifier_pause_samedi() ci-dessus.
                dm, fm, da, fa = h
                for o in occs_travail:
                    if pt_couvre_occurrence(agent, o, pt_blocs):
                        continue
                    if dm is not None and o['debut'] < dm:
                        anomalies.append(Anomalie(
                            'rouge', semaine_label, jour,
                            f"{agent} est indiqué·e en {o['type']} dès {fmt_min(o['debut'])}, "
                            f"avant son arrivée prévue à {fmt_min(dm)} ce samedi.",
                            'Horaires contractuels samedi'))
                    elif fa is not None and o['fin'] > fa:
                        anomalies.append(Anomalie(
                            'rouge', semaine_label, jour,
                            f"{agent} est indiqué·e en {o['type']} jusqu'à {fmt_min(o['fin'])}, "
                            f"après son départ prévu à {fmt_min(fa)} ce samedi.",
                            'Horaires contractuels samedi'))
                verifier_pause_samedi(agent, occs_travail, pause_flex, semaine_label, jour, anomalies)
            else:
                for o in occs_travail:
            ...
            for j in range(i + 1, len(occs)):
                ...
                if a['debut'] < b['fin'] and b['debut'] < a['fin']:
                    if a['type'] == 'Absence' or b['type'] == 'Absence':
                        ...
                        if EXCEPTION_REUNION_ABSENCE in normalize(autre['detail']):
                            continue  # exception validée : réunion pôle, cf. EXCEPTION_REUNION_ABSENCE
                        anomalies.append(Anomalie(
                            'rouge', semaine_label, jour,
    ...
    if planning_type:
        ...
        def _pt_agents(section, cs, ce):
            ...
            Réutilise `pt_blocs` (construit une fois par jour, cf.
            construire_pt_blocs(), partagé avec la règle samedi R1/R4)."""
            for bcs, bce, sections_agents in pt_blocs:
...
def verifier_planning(file_bytes):
    ...
    for sn in semaine_sheets:
        ...
        for jour_data in jours:
            ...
            verifier_jour(jour_data, sn, semaine_num, vue_agent, ALL_AGENTS_CONNUS, prep, anomalies, notes)
